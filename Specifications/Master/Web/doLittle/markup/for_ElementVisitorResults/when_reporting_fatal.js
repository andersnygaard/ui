...
describe("when reporting warning", () => {
    ...
    context.results.fatal(message);

    it("should have a results with the message on it", () => context.results.results[0].message.should.equal(message));
    it("should have a results with fatal severity on it", () => context.results.results[0].severity.should.equal(elementVisitorResultSeverity.fatal));
});